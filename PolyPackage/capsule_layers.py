...
14-11-2021 Modified by Stefano Magni 
- Added support for tensrflow 2.x 
- changed squash function to prevent gradient from dying at tf.norm(0) 
...
class Length(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        if inputs.get_shape()[-2] != self.num_classes:
...
class Mask(layers.Layer):
    ...
    def call(self, inputs, **kwargs):
        ...
        masked_in = inputs*K.expand_dims(mask, -1)
        # masked.set_shape((None,masked.shape[1]*masked.shape[2]))
        masked = K.reshape(masked_in, shape=[-1, masked_in.shape[1]*masked_in.shape[2]])
        #.set-shape=[inputs.shape[0], -1])
...
# ORIGINAL VERSION OF THE SQUASH FUNCTION: IT REQUIRED INPUT PREPROCESSING
def _squash(input_tensor):
    norm = tf.norm(input_tensor, axis=-1, keepdims=True)
    norm_squared = norm * norm
    return (input_tensor / norm) * (norm_squared / (1 + norm_squared))

# SEE SUMMARY FOR EXPLANATION ON THIS NEW VERSION (14-09-2021)
# def _squash(input_tensor, epsilon=1e-7):
    
#     squared_norm = tf.reduce_sum(tf.square(input_tensor), axis=-1,
#                                     keepdims=True)
